...
def get_args():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default='model/yolov7_384x640.onnx',
    )
    parser.add_argument(
        '--score_th',
        type=float,
...
def main():
    ...
    # モデルロード #############################################################
    yolov7_detector = YOLOv7(model_path, conf_thres=score_th, iou_thres=nms_th)
    ...
    while True:
        ...
        # 画面反映 #########################################################
        cv2.imshow('YOLOv7 ByteTrack(Multi Class) Sample', debug_image)
